	...
	if(defined('TOKEN_WEBSERVICE')){
		...
		if(constant('TOKEN_WEBSERVICE')==$objConfiguracion->obtenerTokenWebservices()){	
			...
			if(!empty($arrayParametrosNoValidos)){
				...
				//$objConsultaSQL->addCondicionWhere("zona_comun.id_comunidad",
				//" = '". $parametrosRecibidos['id_comunidad']."' ");
				$objConsultaSQL->addCondicionWhereSecundario("zona_comun.activa",
				" = '". $parametrosObligatorios['zona_activa']."' ");
			...
			if(array_key_exists("id_comunidad", $parametrosRecibidos))
				...
				if(array_key_exists("zona_activa", $parametrosRecibidos))
				$objConsultaSQL->addCondicionWhereSecundario("zona_comun.activa"," = 
				'".$parametrosRecibidos['zona_activa']."' ", clsConstantes::$AND);  

	...
	if(defined('TOKEN_WEBSERVICE')){
		...
		if(constant('TOKEN_WEBSERVICE')==$objConfiguracion->obtenerTokenWebservices()){	
			...
			if(!empty($arrayParametrosNoValidos)){
        		...
        		if(!$objBD->existe('reserva','id',$parametrosRecibidos['id_reserva'])){
			        $response['code'] = 9;
			        $response['status'] = $api_response_code[ $response['code'] ]['HTTP Response'];
			        $response['message'] = $api_response_code[$response['code'] ]['Message'];
			        $response['data']=array(
			       	 		'resultado' => 'id_reserva_no_existe',
			       	 		'id_eliminar' => $parametrosRecibidos['id_reserva'],
							'filas_afectadas' => '0'
			        );
        		}
				else{
					//Eliminamos el registro					
					$sentenciaDelete = "delete from reserva " .
					"where id = '" . $parametrosRecibidos['id_reserva'] . "';";

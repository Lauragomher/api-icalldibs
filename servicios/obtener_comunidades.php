	...
	if(defined('TOKEN_WEBSERVICE')){
		...
		if(constant('TOKEN_WEBSERVICE')==$objConfiguracion->obtenerTokenWebservices()){	
			...
			if(array_key_exists("nombre_comunidad", $parametrosRecibidos))
				$objConsultaSQL->addCondicionWhere("comunidad_de_vecinos.nombre"," like '%".$parametrosRecibidos['nombre_comunidad']."%' ", clsConstantes::$AND);

			if(array_key_exists("id_administrador", $parametrosRecibidos))
				$objConsultaSQL->addCondicionWhere("comunidad_de_vecinos.id_administrador"," = '".$parametrosRecibidos['id_administrador']."' ", clsConstantes::$AND);

	...
	if(defined('TOKEN_WEBSERVICE')){
		...
		if(constant('TOKEN_WEBSERVICE')==$objConfiguracion->obtenerTokenWebservices()){	
			...
			if(!empty($arrayParametrosNoValidos)){
				...
				$objConsultaSQL->addTablaInnerJoin("usuario", "tablon_anuncios.id_usuario = usuario.id");
				...
				$objConsultaSQL->addTablaInnerJoin("comunidad_de_vecinos", "tablon_anuncios.id_comunidad = comunidad_de_vecinos.id", clsconstantes::$AND);
			...
			if(array_key_exists("id_comunidad", $parametrosRecibidos))
				$objConsultaSQL->addCondicionWhere("comunidad_de_vecinos.id_comunidad"," = 

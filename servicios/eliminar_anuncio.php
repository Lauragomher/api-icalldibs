	...
	if(defined('TOKEN_WEBSERVICE')){
		...
		if(constant('TOKEN_WEBSERVICE')==$objConfiguracion->obtenerTokenWebservices()){	
			...
			if(!empty($arrayParametrosNoValidos)){
        		...
        		if(!$objBD->existe('tablon_anuncios','id',$parametrosRecibidos['id_anuncio'])){
			        $response['code'] = 9;
			        $response['status'] = $api_response_code[ $response['code'] ]['HTTP Response'];
			        $response['message'] = $api_response_code[$response['code'] ]['Message'];
			        $response['data']=array(
			       	 		'resultado' => 'id_anuncio_no_existe',
			       	 		'id_eliminar' => $parametrosRecibidos['id_anuncio'],
							'filas_afectadas' => '0'
			        );
        		}
				else{
					//Eliminamos el registro					
					$sentenciaDelete = "delete from tablon_anuncios " .
					"where id = '" . $parametrosRecibidos['id_anuncio'] . "';";
